...
아직 코드가 없는 사전 설계(spec-first) 단계의 저장소다. `docs/`에 두 개의 기획 PDF만 존재한다.

- `docs/TaskFlow_프로그램정의.pdf` — 미션·페르소나·기능 5종·범위 외·DB·API·ACME·비기능 요구사항
- `docs/TaskFlow_스토리보드.pdf` — 42슬라이드 풀버전. 화면 상태/에러 케이스, 통합본 결정 8건, ER·API 명세, 서버·프론트 책임 분담

스토리보드 v2는 프로그램정의 PDF의 빈틈을 **결정 8건**(아래)으로 메운 통합본이며, **두 PDF가 충돌할 경우 스토리보드 v2가 우선한다.** 이 두 문서가 `/opsx:propose`의 입력이 되도록 작성되었으므로, 구현 착수 전 반드시 전체를 읽을 것.
...
코드 작성 후 사용할 로컬 실행 방식:
- 백엔드: `uvicorn main:app --reload` (SQLite 파일 `sqlite:///./taskflow.db`)
- 프론트: `python -m http.server` 또는 live-server로 정적 파일 서빙
- 로컬 SQLite 파일은 git에서 제외할 것

**테스트 자동화는 범위 외다.** pytest/jest 없이 수동 동작 확인만 한다(스토리보드 결정 #7). 성능 기준(API 100ms, 드래그 50ms 반응, 신규 합류 1분 컨텍스트 파악)은 자동 측정 없이 정성 검증한다.
...
- 인덱스: `tasks(team_id, created_at)`(칸반 정렬), `messages(team_id, created_at)`(채팅 폴링 `since=`), `teams(invite_code)` UNIQUE, `users.team_id`.
...
Chat   GET    /teams/{id}/messages     ?since= 증분 폴링(5초). since 없으면 최근 50개
...
- 메시지: 1000자 이내. 채팅 폴링은 5초 간격(모바일 입력 포커스 시 2초), `since=` 마지막 메시지 시각으로 증분 수신.
- 시간대: 서버·클라이언트 모두 KST 가정(UTC 변환 안 함). UI는 한국어만.

## 범위 외 (구현하지 말 것)

알림(이메일/SMS/푸시) · 파일/이미지 첨부 · 전문 검색(단순 SELECT만) · 페이지별 권한 세분화 · 다국어 · WebSocket(5초 폴링으로 대체) · 테스트 자동화 · 로그인 실패 cooldown · JWT 갱신 토큰 · 팀 추방/역할 변경 · 관측성 도구(Sentry/로그 수집, print 디버깅만).
